...
export default class Appointment extends Vue {
  ...
  dragStart (event) {
    ...
    this.$typedStore.commit('drugAndDrop/SET_APPOINTMENT_ID', this.appointment.id)
    this.$typedStore.dispatch('drugAndDrop/fetchTimes')
  }

  async dragEnd (event) {
    this.$typedStore.commit('drugAndDrop/RESET_DRAG')
