...
export default class BarbershopLondon extends Vue {
  ...
  get company () {
    return this.$typedStore.state.widget.company
  }

  get procedure () {
    return this.$typedStore.state.widget.order.procedure
  }

  get specialist () {
    return this.$typedStore.state.widget.order.specialist
  }
  ...
  async submit () {
    try {
      ...
      this.$typedStore.commit('widget/SET_ORDER_BY_KEY', ['date', date])
      this.$typedStore.commit('widget/SET_ORDER_BY_KEY', ['startTime', this.hourSelected])
