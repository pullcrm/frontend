...
export default class Avatars extends Vue {
  ...
  async uploadAvatar (file) {
    ...
    const result = await this.$store.dispatch('specialists/onUploadAvatar', formData)
  ...
  async updateAvatar (avatarId) {
    // @ts-ignore
    await this.$api.specialist.update(this.specialist.id, { avatarId })

    this.$store.dispatch('toasts/show', { title: 'Установлен новый аватар!' })
